...
// npm reports a number of vulnerabilities when installing the 'goodcontracts module.
//const UBIABI = require('@gooddollar/goodcontracts/stakingModel/build/contracts/UBIScheme.min.json');
//const ERC20ABI = require('@gooddollar/goodcontracts/build/contracts/ERC20.min.json');
...
(async () => {
  ...
  console.log('');
  console.log(`Claim offset set to: ${randomMinute}:${randomSecond}`);
  console.log('Offset is to avoid higher transaction fees/instability if everyone claims at once.')
  console.log('');
  cron.schedule(`${randomSecond} ${randomMinute} 0 * * *`, claim, {timezone: "Europe/London"});
})();
